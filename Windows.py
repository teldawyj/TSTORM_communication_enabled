...
class MainWindow:
    ...
    def loop(self):
            ...
            [self.frames, dims] = self.hcam.getFrames()
            print("number of frames : %d"%len(self.frames))
    ...
    def display(self):
        ...
        sleep_time = step * (float(self.ui.exp_t_doublespinbox.text()) ) - display_time
        self.ui.message_label.setText(
            '  get frames number in display:  ' + str(len(self.frames)) + '  max:' + str(self.image_max))
    ...
    def record_state_change(self):
        if not self.ui.recordButton.isChecked():
            ...
            if os.path.exists(self.filename):
                message, ok = QInputDialog.getText(self, "file exists", "continue will append to the old file",
                                                   QLineEdit.Normal,
                                                   "Yes, please append to the old file")
                if not ok:
                    self.ui.recordButton.setText('record')
    ...
    def record(self):
        ...
        # start = time.clock()
        if len(self.frames) > self.record_ability:
            # self.message_label.setText('get frames number' + str(len(self.frames)))
            # print(len(frames))
            for i in self.frames[:self.record_ability]:
                if self.record_thread_flag == False:
                    return (0)
                # start=time.clock()
                image = i.np_array.reshape((2048, 2048))
                try:
                    tiff.imsave(file=self.filename, append=True, data=image, bigtiff=True)
                except:
                    pass
